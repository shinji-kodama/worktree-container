...
func TestGenerateComposeOverride_SingleService(t *testing.T) {
	...
	assert.True(t, strings.HasPrefix(string(result), "# Auto-generated by loam"),
	...
	assert.Equal(t, "loam", appService.Labels["loam.managed-by"],
		"managed-by label should be present")
	assert.Equal(t, "feature-auth", appService.Labels["loam.name"],
...
func TestGenerateComposeOverride_MultiService(t *testing.T) {
	...
	labels := map[string]string{
		"loam.managed-by":     "loam",
		"loam.name":           "feature-multi",
		"loam.config-pattern": "compose-multi",
	}
	...
	for _, svcName := range services {
		...
		assert.Equal(t, "loam", svc.Labels["loam.managed-by"],
			"service %s should have managed-by label", svcName)
		assert.Equal(t, "feature-multi", svc.Labels["loam.name"],
			"service %s should have name label", svcName)
		assert.Equal(t, "compose-multi", svc.Labels["loam.config-pattern"],
...
func TestGenerateComposeOverride_Labels(t *testing.T) {
	...
	labels := map[string]string{
		"loam.managed-by":     "loam",
		"loam.name":           "label-test",
		"loam.branch":         "feature/labels",
		"loam.worktree-path":  "/Users/user/project-label-test",
		"loam.source-repo":    "/Users/user/project",
		"loam.config-pattern": "compose-single",
		"loam.created-at":     "2026-02-28T10:00:00Z",
	}
...
func TestGenerateComposeOverride_ServiceWithoutPorts(t *testing.T) {
	...
	assert.Equal(t, "loam", workerService.Labels["loam.managed-by"],
